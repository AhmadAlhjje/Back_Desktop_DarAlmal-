...
describe('BalanceValuationService', () => {
  it('values FROM_USD_MULTIPLY by division and TO_USD_DIVIDE by multiplication', () => {
    expect(svc.toUsd('47950', '47.95', 'FROM_USD_MULTIPLY')).toBe('1000.00');
    expect(svc.toUsd('1000', '1.135', 'TO_USD_DIVIDE')).toBe('1135.00');
    expect(svc.toUsd('-13513', '13513', 'FROM_USD_MULTIPLY')).toBe('-1.00');
  });
  it('rejects non-positive rates safely', () => {
    expect(svc.toUsd('100', '0', 'FROM_USD_MULTIPLY')).toBe('0.00');
  });
  ...
  it('values USD by its own stored rate, never assuming 1', () => {
    expect(svc.toUsd('100', '2', 'FROM_USD_MULTIPLY')).toBe('50.00');
    expect(svc.toUsd('100', '1.25', 'TO_USD_DIVIDE')).toBe('125.00');
  });
  it('net = us - them with 4 decimals', () => {
    expect(svc.net('1010', '1002')).toBe('8.00');
    expect(svc.net('0', '3200')).toBe('-3200.00');
  });
});
...
describe('GetBalanceSheet', () => {
  ...
  it('valued/simple aggregates per client in USD and hides balanced accounts', async () => {
    ...
    const b = r.rows[1];
    expect(b.totalUs).toBe('100.00'); // 4000 TRY / 40
    expect(b.totalThem).toBe('1002.00');
    expect(b.balanceThem).toBe('902.00');
    expect(r.totals.difference).toBe('108.00'); // 1010 - 902
  });
  it('a USD rate other than 1 changes the valued totals', async () => {
    ...
    expect(r.rows[0].totalUs).toBe('500.00'); // 1000 ÷ 2 وليس 1000
  });
  it('currency/full keeps one row per currency including balanced ones', async () => {
    ...
    expect(r.rows[2].balanceUs).toBe('4000.00');
  });
});
